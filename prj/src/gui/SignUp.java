	...
	private void event() {
		...
		signInNow.addActionListener(e -> {
			...
			if (!check.matcher(id).find()) {
				JOptionPane.showMessageDialog(null, "아이디/비밀번호는 4~20글자사이 영문혹은 숫자 혼합만 가능합니다.", "",
			...
			if (!checkpnum.matcher(pnum).find()) {
				JOptionPane.showMessageDialog(null, "핸드폰번호를 다시 입력해 주세요(010으로 시작, 11자리 숫자)", "",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
